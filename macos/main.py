...
class MainWindow(QMainWindow):
  ...
  def full_test(self):
    ...
    if len(line_address) != 0:
      ...
      address = macos.getAddress(service)
  ...
  def start_test(self):
    ...
    self.ui.status_label.setText(status)
    self.ui.progressBar.setValue(1)
    f.write(macos.fping_dns1() + '\n')
    
    status = "ping dns2"
    self.ui.status_label.setText(status)
    self.ui.progressBar.setValue(15)
    f.write(macos.fping_dns2() + '\n')

    status = "ping yandex"
    self.ui.status_label.setText(status)
    self.ui.progressBar.setValue(30)
    f.write(macos.fping_service("ya.ru") + '\n')
    

    status = "ping google"
    self.ui.status_label.setText(status)
    self.ui.progressBar.setValue(40)
    f.write(macos.fping_service("google.com") + '\n')


    status = "ping steam"
    self.ui.status_label.setText(status)
    self.ui.progressBar.setValue(50)
    f.write(macos.fping_service("store.steampowered.com") + '\n')

    status = "ping blizzard"
    self.ui.status_label.setText(status)
    self.ui.progressBar.setValue(60)
    f.write(macos.fping_service("blizzard.com") + '\n')

    status = "ping vk"
    self.ui.status_label.setText(status)
    self.ui.progressBar.setValue(70)
    f.write(macos.fping_service("vk.com") + '\n')

    status = "ping playstation"
    self.ui.status_label.setText(status)
    self.ui.progressBar.setValue(90)
    f.write(macos.fping_service("store.playstation.com") + '\n')
    f.close()
